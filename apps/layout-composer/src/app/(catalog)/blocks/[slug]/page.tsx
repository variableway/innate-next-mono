...
export default function BlockDetailPage() {
  ...
  const block = getComponentBySlug(slug)
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <Button asChild className="mt-6">
          <Link href="/blocks">Back to Blocks</Link>
        </Button>
      </div>
    )
  }

  // Get related blocks (same subcategory)
  const relatedBlocks = componentRegistry
    .filter((c) => c.category === "block" && c.subcategory === block.subcategory && c.slug !== block.slug)
    .slice(0, 3)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        <Link href="/blocks" className="hover:text-foreground flex items-center gap-1">
          <ChevronLeft className="h-4 w-4" />
          Blocks
        </Link>
        <span>/</span>
        <span className="capitalize">{block.subcategory}</span>
        <span>/</span>
      ...
      {relatedBlocks.length > 0 && (
        <div className="space-y-4 pt-8 border-t">
          <h2 className="text-xl font-semibold">Related Blocks</h2>
          <div className="grid gap-4 md:grid-cols-3">
            {relatedBlocks.map((related) => (
              <Link key={related.slug} href={`/blocks/${related.slug}`}>
...
function CodeView({ block }: { block: { name: string; slug: string } }) {
  const [copied, setCopied] = useState(false)

  // Mock code - in real implementation, this would be the actual component code
  const code = `import { ${block.name} } from "@innate/ui"
    ...
    <Card>
      <CardHeader className="border-b bg-muted/50 flex flex-row items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">{block.name}.tsx</span>
        </div>
...
function TaskView({ block }: { block: { name: string; taskDescription: string } }) {
  const [copied, setCopied] = useState(false)
